...
UPDATED BY  : Arpana Prajapati
10/31 Changes: Error fix for s.hier1 pop-up, s.prop11 for s_code version, event 13 for tracking alerts
11/05 Changes: Expected string error at Line 348 of Media module - removed trailing comma
11/21 Changes: Added .txt extension in s.linkDownloadFileTypes
11/27 Changes: Added prop32/evar32 for download filetype and edited prop33/evar33 values to strip filenames from url
12/1  Changes: Added Plugin: getTimeParting 3.3 that Jason Cook provided.
5/1/13 CW: Set eVar7 even when pageName was already set, send event3 with download
6/3/13 CW: Set campaign variable to prop41 before deduping, lower case user id, remove query string from pageName
8/21/13 CW: Don't check if eVar7 is already set.  This causes problems on dynamic sites without page reloads.
6/18/14 CW: Updated getPageName to clean up case diffs and special characters; also added semicolons to the end of lines
*/
...
/************************ ADDITIONAL FEATURES ************************
     ...
     Plugins
*/
//var s_account = window.s_account = "deloitteus-dnet-dev";
...
s.linkDownloadFileTypes="txt,exe,zip,wav,mp3,mov,mpg,avi,wmv,pdf,doc,docx,xls,xlsx,ppt,pptx";
s.linkInternalFilters="javascript:,deloitte.com,deloittenet.com"; //Edit this for app-specific domain
s.linkLeaveQueryString=false;
s.linkTrackVars="server,events,eVar1,eVar2,eVar7,eVar14";
...
function s_doPlugins(s) {
	...
	//Update prop11 to last updated date whenever editing this file
	s.prop11="v06.18.14"; 
	...
	//Page Name
	if(!s.pageName) {
		s.pageName = getPageName(document.URL);
	}
	s.eVar7 = s.pageName;
	...
	//Time Parting 
	var tpA = s.getTimeParting('s','-5');
	s.prop1 = s.eVar1 = tpA[1]; // Time at 30 minute level
	s.prop2 = s.eVar2 = tpA[2]; // Day of week
	s.prop3 = s.eVar3 = tpA[3]; // Weekend or Weekday
	
	//New/Repeat Visitors
	s.prop13 = s.eVar13 = s.getNewRepeat();
	
	//Visit Number
	s.prop4 = s.eVar4 = s.getVisitNum();
	
	//Days Since Last Visit
	s.prop9 = s.eVar9 = s.getDaysSinceLastVisit('s_lv');
	
	//Content Title
	s.prop7 = s.eVar41 = document.title;
	
	//URL
	s.prop75 = s.eVar75 = document.location.href;
	
	//Download Links
	var dllink=s.downloadLinkHandler();
	if(dllink){
	    s.prop31 = s.eVar31 =s.pageName;
	    s.prop33 = s.eVar33 = dllink.substring(dllink.lastIndexOf("/") + 1, dllink.length);
	    s.eVar32 = dllink.substring(dllink.lastIndexOf(".") + 1, dllink.length);
	    s.eVar68 = s.prop68 = dllink.substring(dllink.lastIndexOf("/") + 1, dllink.length);
		s.events = s.apl(s.events, 'event3', ',', 2);
		s.linkTrackVars = s.apl(s.linkTrackVars, 'prop31,prop33,eVar33,eVar32,eVar31,s.prop31,prop65,eVar65,prop75,eVar75,prop14,eVar14,eVar68,prop68,events', ',', 2);
		s.linkTrackEvents=s.apl(s.linkTrackEvents,'event3',',',2);
		if(s.campaign){
			s.eVar33=s.campaign;
			s.linkTrackVars=s.apl(s.linkTrackVars,'eVar33',',',2);
		}
	}
	...
	if (s_hierStr){
		if (s_hierStr[1]) {
			s.hier1=decodeURI(s_hierStr[1].toLowerCase());
		}
	}
	...
	if(s.hier1){
		...
		if(hsp[1]) s.prop36 = s.eVar36 = hsp[1]; //Site Sub Section 2
		if(hsp[2]) s.prop37 = s.eVar37 = hsp[2]; //Site Sub Section 3
		if(hsp[3]) s.prop38 = s.eVar38 = hsp[3]; //Site Sub Section 4
		if(hsp[4]) s.prop39 = s.eVar39 = hsp[4]; //Site Sub Section 5
		if(hsp[5]) s.prop40 = s.eVar40 = hsp[5]; //Site Sub Section 6	
		if(s.channel)
			s.prop48 = s.eVar48 = s.channel; //Site Sub Section 1
		else if(hsp[0]) 
			s.channel = s.prop48 = s.eVar48 = hsp[0]; //Site Sub Section 1
	}
	
	// Storing previous pagename
	s.prop50 = s.getPreviousValue(s.pageName,'s_gpv_p50');

	// Track Alerts 
	if(s.getQueryParam('AlertID')){
		s.events=s.apl(s.events,"event13",",",2); 
		s.eVar53=s.prop53=s.getQueryParam('AlertID');
	}

	if(s.prop14) {
		s.prop14 = s.eVar14 = s.visitorID = s.prop14.toLowerCase();
	} else if (s.eVar14) {
		s.prop14 = s.eVar14 = s.visitorID = s.eVar14.toLowerCase();
	}
...
/* Friendly Page Name  custom plug-in for Deloitte */	
function getPageName(URL) {
	var j = 0;

	// First, remove query strings, then break down the URL into separate components using "/" as a separater
	var pageNameString = URL.split("?");
	pageNameString = pageNameString[0].split("/");
	for (var i=0; i < pageNameString.length; i++) { 
		// Ignore http:, https:, null strings and meaningless words like "Pages"
		if (pageNameString[i].toLowerCase() == "pages" || pageNameString[i].toLowerCase() == "http:" || pageNameString[i].toLowerCase() == "https:" || pageNameString[i] == "") {
			continue;
		} else {
			//Capitalize first letter for easy readability in friendly page names
			 pageNameString[j] = pageNameString[i].charAt(0).toUpperCase() + pageNameString[i].slice(1);
			j++;
		}
	}

	//Usually, just the first part of the domain name is relevant. For example: for the current set of pages, first section is deloittenet
	if (pageNameString[0].indexOf(".") != -1) {
		pageNameString[0] = pageNameString[0].substring(0,pageNameString[0].indexOf("."));
	}
	
	// If the last string is of format, pagename.aspx or pagename.html, Drop the extension and keep just the pagename
	if (pageNameString[j-1].indexOf(".") != -1) {
		pageNameString[j-1] = pageNameString[j-1].substring(0,pageNameString[j-1].lastIndexOf("."));
	}

	pageNameString.splice(j,i-j);
	return decodeURI(pageNameString.join("|"));
}
...
s.Media.trackVars="events,prop47,eVar43,eVar44,eVar45";
s.Media.trackEvents="event19,event20,event21,event22,event23,event24,event25";
s.Media.trackMilestones="25,50,75,99";
s.Media.playerName="Default Player";
s.Media.segmentByMilestones=true;
s.Media.trackUsingContextData=true;
s.Media.contextDataMapping={
  "a.media.name":"eVar43,prop47",
  "a.media.segment":"eVar44",
  "a.contentType":"eVar45",
